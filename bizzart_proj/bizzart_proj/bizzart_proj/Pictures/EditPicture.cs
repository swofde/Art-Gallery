    ...
    public partial class EditPicture : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            using (ArtGalletyContext db = new ArtGalletyContext())
            {
                ...
                {
                    db.Pictures.Where(pic1 => pic1.Id == pic.Id).FirstOrDefault().Artist = comboBoxArtist.SelectedItem as Artist;
                }
                if ((comboBoxDepartment.SelectedItem as Department).Id != pic.DepartmentId)
                {
                    db.Pictures.Where(pic1 => pic1.Id == pic.Id).FirstOrDefault().Department = comboBoxDepartment.SelectedItem as Department;
                }
                db.SaveChanges();
